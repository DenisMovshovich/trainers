...
const FILE=rel("../sql-offline.html");
const b=await chromium.launch({executablePath:EXE});
let bad=0;
for(const [w,h] of [[1500,980],[1440,900],[1200,780],[1100,700]]){
  ...
  const r=await p.evaluate(()=>{
    ...
    return {railBottom:Math.round(rb.bottom), dockTop:Math.round(db.top), btnBottom:Math.round(bb.bottom),
            ...
            schScroll:document.querySelector(".sch").scrollHeight>document.querySelector(".sch").clientHeight};
  });
  const overlap = r.btnBottom > r.dockTop;
  if(overlap) bad++;
