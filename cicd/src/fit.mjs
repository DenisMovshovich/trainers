...
for(const [w,h] of [[1540,1000],[1440,900],[1280,820],[1200,760],[1160,700],[1121,660]]){
  ...
  await p.fill("#cmd", "ci run"); await p.press("#cmd", "Enter"); await p.waitForTimeout(250);
  await p.evaluate(()=>window.scrollTo(0,1400)); await p.waitForTimeout(200);
  const r=await p.evaluate(()=>{
    const rail=document.querySelector(".rail"), dock=document.querySelector(".dock"), gl=document.querySelector("#jobs");
  ...
  if(overlap||tiny) bad++;
  console.log(w+"×"+h+" | --dockh="+r.dockh+" (факт "+r.dockH+") | низ списка "+r.railBottom+
    " · верх панели "+r.dockTop+" → "+(r.hidden?"колонка скрыта":overlap?"ПЕРЕКРЫТИЕ":"ок")+
    " | высота списка "+r.glH+(tiny?" — СЛИШКОМ МАЛО":""));
  await ctx.close();
}
