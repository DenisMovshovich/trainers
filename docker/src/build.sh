...
#   ../docker.html          — исходник для публикации артефактом (без doctype/head/body,
#                                  их добавляет публикация; этот путь менять нельзя,
#                                  иначе артефакт получит новый URL)
#   ../docker-offline.html  — автономный документ для локального открытия в браузере
set -e
cd "$(dirname "$0")"
...
python3 - <<'PY'
src = open("../docker.html", encoding="utf-8").read()
...
out = ('<!doctype html>\n<html lang="ru">\n<head>\n<meta charset="utf-8">\n'
       ...
       + src[:i] + '\n</head>\n<body>' + src[i:] + '\n</body>\n</html>\n')
open("../docker-offline.html", "w", encoding="utf-8").write(out)
PY

echo "артефакт:  $(wc -c < ../docker.html) байт"
echo "автономно: $(wc -c < ../docker-offline.html) байт"
