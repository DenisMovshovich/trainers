...
const CODE = ["30_yaml.js","31_expr.js","32_runner.js","33_engine.js","34_cli.js","35_disp.js"].map(strip).join("\n");
const CONTENT = fs.readdirSync(dir).filter(x => /^4\d_content_.*\.js$/.test(x)).sort().map(strip).join("\n");
const CTX = new Function(CODE + "\n" + CONTENT +
  "\nreturn {MODULES, parseYaml, newScenario, runCi, parseWorkflow, workflowFile};")();
const {MODULES, parseYaml, newScenario, runCi, parseWorkflow, workflowFile} = CTX;
...
/* Сценарий должен строиться без ошибок и оставлять репозиторий в осмысленном виде. */
{
  ...
  for(const m of MODULES) for(const lab of m.labs){
    ...
    try{
      const C = lab.setup();
      if(!C || !C.files) { err("задание " + lab.id + ": сценарий не вернул проект"); continue; }
      files += Object.keys(C.files).length;
      /* YAML сценария обязан разбираться — кроме заготовок, которые пишет ученик */
      for(const f in C.files){
        if(!/\.ya?ml$/.test(f)) continue;
        if(/^\s*#/.test(C.files[f]) && C.files[f].split("\n").filter(x => x.trim()).length <= 1) continue;
...
if(!cm) err("в 50_app.js не найден список примеров CHIPS");
  ...
  for(const id in CHIPS){
    ...
    for(const c of CHIPS[id]){
      ...
      const C = newScenario({files:{"src/App.cs":"class App { }",
        ".github/workflows/ci.yml":"name: CI\non: [push]\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - run: dotnet build"}});
      const r = runCi(C, cmd);
      if(r.err && /не живёт|нет команды|незакрытая/.test(r.err))
