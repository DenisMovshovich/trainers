...
/* ============================================================
   ...
   ============================================================ */
class K8sErr extends Error {
  constructor(msg, hint){ super(msg); this.hint = hint || ""; }
}
const kerr = (m, h) => { throw new K8sErr(m, h); };
...
function parseYaml(src){
  ...
  for(const chunk of String(src).split(/^---\s*$/m)){
    ...
    docs.push(val);
    if(p < rows.length) kerr("не разобрана строка " + rows[p].line + ": «" + rows[p].text + "»");
  }
  return docs;
}
...
function parseList(rows, get, set, indent){
  ...
  while(get() < rows.length){
    ...
    if(r.indent > indent) kerr("странный отступ в строке " + r.line,
    ...
    if(c > 0 && (r.text[c + 1] === " " || c === r.text.length - 1)){
      ...
      while(get() < rows.length && rows[get()].indent >= inner && !rows[get()].dash){
        const q = rows[get()];
        if(q.indent > inner){ kerr("странный отступ в строке " + q.line); }
        const ci = q.text.indexOf(":");
        if(ci < 0) kerr("ожидалось «ключ: значение» в строке " + q.line);
...
function parseMap(rows, get, set, indent){
  ...
  while(get() < rows.length){
    ...
    const c = r.text.indexOf(":");
    if(c < 0) kerr("ожидалось «ключ: значение» в строке " + r.line + ": «" + r.text + "»",
