...
eval(eng + "\n" + content +
     "\nfor(const n of [\"MODULES\",\"runSh\",\"newMachine\",\"statPath\",\"readFile\",\"writeFile\",\"syncProcs\"]) globalThis[n] = eval(n);");
...
for(const m of MODULES){
  for(const lab of m.labs || []){
    ...
    const push = cmd => {
      const r = runSh(S, cmd);
      st.log.push({cmd, out: (r.out || []).slice(), err: r.err || null});
      return r;
    };
    ...
    if(!neg){
      for(const step of steps){
        ...
        if(step.file !== undefined){
          writeFile(S, step.file, step.text, {who: {uid: 0, gids: [0], name: "root"}});
          continue;
        }
        if(step.cmd){
    ...
    for(const c of lab.checks){
      ...
      if(neg && pass)  bad.push(lab.id + " / " + plain + " — проходит на нетронутой машине");
    }
  }
}
