...
const {MODULES, GLOSSARY} = new Function(CODE+"\n"+CONTENT+"\nreturn {MODULES, GLOSSARY};")();
...
MODULES.forEach((m, i) => {
  ...
  if(!m.iv || m.iv.length < 2) err("модуль " + m.n + ": меньше двух карточек собеседования");
  (m.iv || []).forEach((c, ci) => {
    const w = "модуль " + m.n + " собеседование " + (ci + 1);
    ["q","probe","a"].forEach(k => { if(!c[k]) err(w + ": нет поля " + k); });
    if(c.a && c.a.length < 200) err(w + ": слишком короткий модельный ответ");
    balance(c.q, w + " вопрос"); balance(c.a, w + " ответ"); balance(c.probe, w + " подсказка");
  });
  m.labs.forEach(l => {
    nl++;
    ...
    if(!l.ex) err(w + ": нет упражнения");
    else {
      const K = ["values","classify","pick","fields"];
      if(K.indexOf(l.ex.kind) < 0) err(w + ": неизвестный вид упражнения " + l.ex.kind);
      if(l.ex.kind === "classify" && (!l.ex.options || !l.ex.items)) err(w + ": у classify нет options или items");
      if(l.ex.kind === "pick" && !l.ex.items) err(w + ": у pick нет items");
      if(l.ex.kind === "fields" && !l.ex.fields) err(w + ": у fields нет полей");
      if(l.ex.items) l.ex.items.forEach((it, i) => balance(it, w + " пункт " + (i + 1)));
    }
...
console.log("── глоссарий");
{
  ...
  GLOSSARY.forEach((g, i) => {
    if(!g[0] || !g[1]) err("глоссарий " + i + ": пустой термин или определение");
    if(seen.has(g[0])) err("глоссарий: повтор термина «" + g[0] + "»");
    seen.add(g[0]);
    balance(g[1], "глоссарий «" + g[0] + "»");
    if(g[2]) balance(g[2], "глоссарий «" + g[0] + "» ловушка");
  });
